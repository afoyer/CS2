public class DoubleLinkList<T> implements IList<T>{
    ...
    public void insert(int idx, T v) {
        ...
        if (size ==0) {
            IDLink<T> first = new Nodes(v, null, null);
            first.setPrev(first);
            first.setNext(first);
            head = first;
            tail = first;
            ...
            if (idx == size) {
                ...
                if (idx == 0) {
                    head.setNext(head);
                    IDLink<T> tc = new Nodes(v, null, head);
                    tc.setPrev(tc);
                    head.setPrev(tc);
                    this.head = tc;
                }
                else if (idx == 1){
                    IDLink<T> tc = new Nodes(v, this.head, this.head.getNext());
                    this.head.setNext(tc);
                }
                else {
                    for (int i = 0; i < size; i++) {
                        if (i == idx) {
                            IDLink<T> tc = new Nodes(v, head.getPrev(), head);
